 ...
 */
export async function uploadImage(eventid, content, metadata) {
  ...
  const src = await uploadToS3(eventid, filename, buffer, contentType);
  ...
  await uploadToFirestore(eventid, filename, fullData);
  return fullData;
}